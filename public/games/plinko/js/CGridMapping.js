...
function CGridMapping(aMatrix) {
  ...
  this._getStraightByDirAndRadius = function (
    iRow,
    iCol,
    szDir,
    iRadius,
  ...
  this._findInDirection = function (
    iRow,
    iCol,
    iDir,
    aList,
  ...
  this.getAllPathFromTo = function (oStartPos, oEndPos) {
    ...
    for (var i = aAllPath.length - 1; i >= 0; i--) {
      ...
      for (var j = 0; j < aAllPath[i].length; j++) {
        if (
          aAllPath[i][j].row === oEndPos.row &&
          aAllPath[i][j].col === oEndPos.col
        ) {
          bNodeFound = true;
          break;
        }
  ...
  this._getChildren = function (aNode) {
    ...
    for (var key in aNode) {
      if (
        aNode[key] !== null &&
        (key === DIR_BOTLEFT || key === DIR_BOTRIGHT)
      ) {
