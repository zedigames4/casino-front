function CBallGenerator(oParentContainer) {
  ...
  this._init = function (oParentContainer) {
    ...
    for (var i = 0; i < _iBallInTube; i++) {
      var oBallPos = {
        x: _pStartPoint.x - i * _iOffsetFromBall,
        y: _pStartPoint.y,
      };
  ...
  this.shiftBallAnimation = function () {
    ...
    var oBallPos = {
      x: _pStartPoint.x - iLastIndex * _iOffsetFromBall,
      y: _pStartPoint.y,
    };
    ...
    for (var i = 0; i < _iBallInAnimation; i++) {
      ...
      createjs.Tween.get(_aBall[i].getSprite(), { override: true })
        .wait(i * 200)
        .to({ x: oBallPos.x }, 1000, createjs.Ease.cubicIn);
    }
  };
