...
function CGame(oData) {
  this._init = function () {
    ...
    _iMultiply = 1;
    _iCurBet = START_BET / 1;
    _iCurCredit = START_CREDIT / 1;
  ...
  this._setBoard = function () {
    ...
    _oBoardContainer.regX =
      _oBoardContainer.getBounds().x + _oBoardContainer.getBounds().width / 2;
    _oBoardContainer.regY =
      _oBoardContainer.getBounds().y + _oBoardContainer.getBounds().height / 2;
  ...
  this.setBall = function () {
    ...
    var oNewPos = _oBoardContainer.globalToLocal(
      oCurBallPos.x * s_iScaleFactor,
      oCurBallPos.y * s_iScaleFactor,
    );
  ...
  this.getFallPath = function () {
    ...
    var aPath = s_oGridMapping.getRandomPathFromColToCol(
      _iColToLaunchBall,
      iEndCol,
    );
  ...
  this.ballArrived = function (iDestCol) {
    ...
    iProfit = (PRIZE[iDestCol] * _iMultiply) / _iCurBet;
    _oScoreBasketController.litBasket(iDestCol, iProfit);
    _iWin = PRIZE[iDestCol] * _iMultiply;
  ...
  this._setEndCol = function () {
    ...
    var iPrizeToChoose =
      aAllPossiblePrize[Math.floor(Math.random() * aAllPossiblePrize.length)]
        .index;
