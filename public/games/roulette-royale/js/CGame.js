...
function CGame(oData) {
  this._init = function () {
    ...
    _oTableController.addEventListener(
      ON_SHOW_BET_ON_TABLE,
  ...
  this._startBallSpinAnim = function () {
    ...
    _oWheelAnim.startSpin(
      0,
      s_oGameSettings.getFrameForBallSpin(0, _iNumberExtracted),
      _iNumberExtracted,
      iWin,
    );
  };
  ...
  this._generateWinLoss = function () {
    ...
    _iCasinoCash = parseFloat(_iCasinoCash);
  ...
  this._onShowBetOnTable = function (oParams, bRebet) {
    ...
    if (!bRebet) {
      ...
      _aRebetHistory.push({
        button: oParams.button,
        numbers: oParams.numbers,
        bet_mult: oParams.bet_mult,
        bet_win: oParams.bet_win,
        num_fiches: oParams.num_fiches,
        neighbors: false,
        value: iIndexFicheSelected,
      });
    } else {
    ...
    switch (szBut) {
      ...
      case 'oBetFinalsBet': {
        _oMySeat.createPileForMultipleNumbers(
          iFicheValue,
          iIndexFicheSelected,
          aNumbers,
          iBetWin,
  ...
  this._onShowBetOnTableFromNeighbors = function (oParams, bRebet) {
    ...
    if (!bRebet) {
      ...
      _aRebetHistory.push({
        ...
        bet_win: oParams.bet_win,
        num_fiches: oParams.num_fiches,
        value: _oInterface.getCurFicheSelected(),
        num_clicked: oParams.num_clicked,
        neighbors: true,
      });
    }
  ...
  this.onOpenNeighbors = function () {
    _oFinalBet.hide();
    _oNeighborsPanel.showPanel(
      _oInterface.getCurFicheSelected(),
      _oMySeat.getCredit(),
      _oMySeat.getCurBet(),
    );
  };
  ...
  this._updateDistributeFiches = function () {
    ...
    if (_iTimeElaps > TIME_FICHES_MOV) {
      ...
      var fLerp = s_oTweenController.easeInOutCubic(
        _iTimeElaps,
        0,
        1,
        TIME_FICHES_MOV,
