function CInterface() {
  ...
  this._init = function () {
    ...
    _oBetVoisinsZero = new CBetTextButton(
      266,
      641,
    ...
    _oBetOrphelins = new CBetTextButton(
      582,
      641,
      s_oSpriteLibrary.getSprite('but_bg'),
      TEXT_ORPHELINS,
    ...
    _oBetNeighbors = new CTextButton(
      740,
      641,
      s_oSpriteLibrary.getSprite('but_bg'),
      TEXT_NEIGHBORS,
    ...
    _oBetFinalsBet = new CTextButton(
      898,
      641,
      s_oSpriteLibrary.getSprite('but_bg'),
      TEXT_FINALSBET,
    ...
    try {
      /*menu icon*/
      try {
        oSprite = s_oSpriteLibrary.getSprite('menu_icon');
        _pStartPosMenu = {
          x: CANVAS_WIDTH - oSprite.width / 2 - 10,
          y: oSprite.height / 2 + 60,
        };
  ...
  this.refreshButtonPos = function (iNewX, iNewY) {
    ...
    _oMenuButton.setPosition(
      _pStartPosMenu.x - iNewX,
      _pStartPosMenu.y + iNewY,
    );
  };
  ...
  this._initFichesBut = function () {
    ...
    for (var i = 0; i < NUM_FICHES; i++) {
      ...
      _aFiches[i].addEventListenerWithParams(
        ON_MOUSE_UP,
  ...
  this.setMoney = function (iMoney) {
    _oMoneyAmountText.refreshText(iMoney + TEXT_CURRENCY);
  };

  this.setCurBet = function (iCurBet) {
    _oBetAmountText.refreshText(iCurBet + TEXT_CURRENCY);
  };

  this.refreshMoney = function (iStartMoney, iMoney) {
    _oRollingText = new CRollingTextController(
      _oMoneyAmountText.getText(),
      null,
      iStartMoney,
      parseFloat(iMoney),
      1000,
