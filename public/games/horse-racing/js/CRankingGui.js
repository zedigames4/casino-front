function CRankingGui(aNames, oParentContainer) {
  ...
  this._init = function (aNames) {
    ...
    _oRadarMask.graphics
      .beginFill('rgba(255,255,255,0.01)')
      .drawRect(_oFillRadar.x, _oFillRadar.y - 2, 0.01, 10);
  ...
  this.refreshRank = function (aRank) {
    for (var i = 0; i < aRank.length; i++) {
      ...
      createjs.Tween.get(_aBibs[iBibIndex]).to(
        { x: _aPlacePos[i].x },
        1000,
  ...
  this.refreshRadar = function (iNum) {
    ...
    _oRadarMask.graphics
      .beginFill('rgba(255,255,255,0.01)')
      .drawRect(_oFillRadar.x, _oFillRadar.y - 2, _iStepMask * iNum, 10);
  };
