  ...
  this._placeForecastBetForHorse = function (iHorseIndex, iX, iY) {
    ...
    for (var j = 0; j < NUM_HORSES; j++) {
      if (j !== iHorseIndex) {
        ...
        var oTextOdd = new createjs.Text(
          s_oGameSettings.getForecastOdd(iHorseIndex, j),
          '18px ' + PRIMARY_FONT,
          '#fff',
        );
        ...
        var oBut = new CButBet(
          oOddBg.x + oSpriteOddBg.width + (oSpriteButton.width * 0.72) / 2 + 5,
          oOddBg.y + (oSpriteButton.height * 0.72) / 2 - 2,
          oSpriteButton,
          0.45,
          _oContainer,
        );
  ...
  this._onForecastClicked = function (oParams) {
    ...
    if (
      s_oBetPanel.setForecastBet(
        ...
        oParams.second,
        iFicheValue,
        _aForecastBut['forecast_' + oParams.first + '_' + oParams.second],
      )
    ) {
      _aForecastBut[
        'forecast_' + oParams.first + '_' + oParams.second
      ].increaseBet(iFicheValue);
    }
  };
