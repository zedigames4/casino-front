function CFichesController(iScale, oParentContainer) {
  ...
  this._init = function (iScale) {
    ...
    _oBetAmountBackText = new createjs.Text(
      '',
      szFontSize + 'px ' + PRIMARY_FONT,
      '#000',
    );
    _oBetAmountBackText.textAlign = 'center';
    _oTextAttach.addChild(_oBetAmountBackText);

    _oBetAmountText = new createjs.Text(
      '',
      szFontSize + 'px ' + PRIMARY_FONT,
      '#fff',
    );
  ...
  this.refreshFiches = function (aFiches, iXPos, iYPos) {
    ...
    for (var i = 0; i < aFiches.length; i++) {
      var oSpriteChip = new CFicheBut(
        aFiches[i].index,
        iXOffset,
        iYOffset,
        iScale,
        _oFichesAttach,
      ); //s_oSpriteLibrary.getSprite("fiche_"+aFiches[i].index);
      oSpriteChip.disable();
  ...
  this.createFichesPile = function (iAmount, iX, iY) {
    ...
    do {
      ...
      for (var i = 0; i < iNumFiches; i++) {
        aFichesPile.push({
          value: iMinValue,
          index: s_oGameSettings.getIndexForFiches(iMinValue),
        });
      }
