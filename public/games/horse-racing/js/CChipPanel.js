function CChipPanel(iX, iY, oParentContainer) {
  ...
  this._initChips = function () {
    ...
    for (var i = 0; i < NUM_CHIPS; i++) {
      ...
      _aChipButtons[i].addEventListenerWithParams(
        ON_MOUSE_UP,
  ...
  this._onStartRace = async function () {
    const module = await import('../../dev/src/system/GlobalFunctions.js');
    await module.finishBet(ids.horseRacing.id);
